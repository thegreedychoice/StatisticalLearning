"""
This performs Supervised Nearest Centroid classification on the Iris Data Set into 3 classes.
Will plot decision boundaries for each class
"""
...
for shrinkage in [None, .2]:
	...
	clf = neighbors.NearestCentroid(shrink_threshold=shrinkage)
	#Fit the Training Data on the classifier
	clf.fit(X, y)
	y_pred = clf.predict(X)
	print(shrinkage, round(np.mean(y_pred==y), 2))
	...
	plt.title("Iris 3-Class Neighbors Centroid Classification (shrinkage = %r)"%shrinkage)
	plt.axis('tight')
